...
export async function runMessageAction(
  ...
  const currentChannel = normalizeMessageChannel(input.toolContext?.currentChannelProvider);
  if (!explicitChannel) {
    if (currentChannel && isDeliverableMessageChannel(currentChannel)) {
      params.channel = currentChannel;
    } else if (currentChannel === INTERNAL_MESSAGE_CHANNEL) {
      throw new Error(
        "Message tool cannot auto-route to internal webchat. Reply normally in this session, or pass an explicit deliverable channel.",
      );
    }
  }
